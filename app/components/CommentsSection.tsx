       ...
       {comments.filter((comment) =>!comment.replyToId).map((topLevelComment) => {
        ...
        return <div key={topLevelComment.id} className="flex flex-col">
          ...
          .map((reply) => {
            ...
            return <div key={reply.id} className="ml-2 py-2 pl-4 border-l-2 border-neutral-800">
             <PostComment 
             comment={reply} currentVote={replyVote} votesAmt={replyVotesAmt} postId={postId}/>
            </div>
          })}
        </div>
       })}
